# Chapter 48: Debugging Angular Applications
...
The Angular DevTools browser extension (available for Chrome and Edge) is the single most productive diagnostic tool in the Angular ecosystem. [Chapter 24](ch24-performance.md) introduced the Profiler tab for performance work; the extension does more than that. Install it from the Chrome Web Store, open DevTools, and a new **Angular** panel appears next to Elements and Console.
...
Hydration ([Chapter 17](ch17-defer-ssr-hydration.md)) is brittle by nature: the server renders HTML, the client renders the same component tree, and if the two do not agree on structure, Angular throws an `NG0500`-series error. The error message lists the mismatched node, the expected structure, and the actual DOM at the point of divergence.

Common causes in FinancialApp:

- **Conditional rendering based on `window` or `document`.** A component that renders one tree on the server (where `window` is undefined) and a different tree on the client will fail to hydrate. Use the DI-based platform abstraction from [Chapter 17](ch17-defer-ssr-hydration.md) instead of direct `window` checks, so both environments render the same structure.
...
Zoneless change detection ([Chapter 24](ch24-performance.md)) eliminates a whole class of bugs -- and introduces a smaller, more specific class in exchange. The most common symptom is "the template does not update when I change this signal." There are two usual culprits.
