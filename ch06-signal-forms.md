...
Signal Forms is the v21 default and what the rest of this chapter (and this book) uses, but Reactive Forms -- the `FormControl` / `FormGroup` / `FormBuilder` trio from `@angular/forms` -- still powers a huge amount of existing code. If you maintain an application from an earlier Angular major, or consume a third-party library whose public API exposes `FormControl`, you need to be fluent in both. This section is a compact reference: just enough Reactive Forms to read existing code, plus the interop patterns that let a signal-first component host a legacy form without friction.

For the authoritative reference, the [Angular Forms guide](https://angular.dev/guide/forms) covers the full API surface. The goal here is to cover the 80% of patterns you will actually encounter.

### FormControl, FormGroup, FormArray, FormBuilder

A `FormControl` wraps a single value with change tracking and validation:

```typescript
import { FormControl, Validators } from '@angular/forms';

const amount = new FormControl<number | null>(null, {
  validators: [Validators.required, Validators.min(0.01)],
  nonNullable: false,
});

amount.setValue(100);
console.log(amount.value);      // 100
console.log(amount.valid);      // true
console.log(amount.errors);     // null
```

`FormGroup` composes several controls into a keyed structure:

```typescript
import { FormControl, FormGroup, Validators } from '@angular/forms';

const transferForm = new FormGroup({
  amount: new FormControl<number | null>(null, [Validators.required, Validators.min(0.01)]),
  recipient: new FormControl('', [Validators.required, Validators.minLength(3)]),
  memo: new FormControl(''),
});

console.log(transferForm.value);   // { amount: null, recipient: '', memo: '' }
console.log(transferForm.valid);   // false
```

`FormArray` holds a dynamic list of controls or groups -- useful for "add another line item" patterns:

```typescript
import { FormArray, FormControl } from '@angular/forms';

const tags = new FormArray([
  new FormControl('housing'),
  new FormControl('recurring'),
]);

tags.push(new FormControl('tax-deductible'));
```

`FormBuilder` is a small service that reduces the `new FormControl(...)` boilerplate:
...
`fb.group({ field: [initialValue, validators] })` is shorthand for constructing `FormControl` instances. `fb.array([...])` and `fb.control(value, validators)` cover the other two shapes.

### Binding a Reactive Form to a Template

The component imports `ReactiveFormsModule` and the template wires inputs to controls with `formControlName` (inside a `formGroup`) or `[formControl]` (standalone):
...
<form [formGroup]="form" (ngSubmit)="submit()">
  <label>
    ...
    @if (form.controls.amount.invalid && form.controls.amount.touched) {
      <span class="error">Amount must be at least $0.01.</span>
    }
  </label>
...
Template-driven forms are the other legacy flavor -- forms where the model is built implicitly from `ngModel` directives in the template. They import `FormsModule` (not `ReactiveFormsModule`) and bind with two-way syntax:
...
@Component({
  ...
  imports: [FormsModule],
  template: `
    <input type="text" name="note" [(ngModel)]="note" required />
    <p>You typed: {{ note }}</p>
  `,
})
...
Template-driven forms are fine for trivial one-off inputs. For anything more complex -- multiple validators, dynamic field lists, cross-field validation -- Reactive Forms is the better option among the legacy approaches, and Signal Forms supersedes both for new code.

### Built-in and Custom Validators

`Validators.required`, `Validators.minLength(n)`, `Validators.maxLength(n)`, `Validators.min(n)`, `Validators.max(n)`, `Validators.pattern(regex)`, and `Validators.email` cover the common cases. A custom validator is a function that returns `null` for a valid control or an error object:

```typescript
import { AbstractControl, ValidationErrors, ValidatorFn } from '@angular/forms';

export function multipleOf(step: number): ValidatorFn {
  return (control: AbstractControl): ValidationErrors | null => {
    const value = control.value;
    if (value == null || value === '') return null;
    return value % step === 0 ? null : { multipleOf: { requiredStep: step } };
  };
}

// usage
amount: [null, [Validators.required, multipleOf(0.01)]],
```

Async validators (for example, "is this email already registered?") return an Observable or Promise of `ValidationErrors | null` and plug in via the third argument to `new FormControl` or the `asyncValidators` key in `FormBuilder`.

### Migrating Reactive Forms to Signal Forms

The conceptual mapping is cleaner than it looks:
...
| `Validators.required` | `validators.required()` / Zod `.min(1)` |
| `control.updateValueAndValidity()` | implicit -- signals re-evaluate automatically |
| `form.markAllAsTouched()` | `form.markAllAsTouched()` (same name, signal-backed) |
| `[formGroup]` / `formControlName` | `[formField]="form.field"` |

Practical migration approach:

1. Convert the topmost `FormBuilder` call to `formField`/`formGroup`/`formArray`.
2. Replace each `formControlName` binding with `[formField]="form.fieldName"`.
3. Move validators into the signal-forms schema or Zod, one field at a time.
4. Replace `valueChanges.subscribe(...)` with a `computed()` or `effect()` reading `field.value()`.

Because `ControlValueAccessor` components work with both worlds (see *Custom Fields* earlier in this chapter), you can migrate form-by-form rather than all at once.

### Interop: Reading a Reactive Form from a Signal-Based Component

Sometimes the migration is incremental and you need a signal-based parent to consume a Reactive `FormControl` exposed by a child or a library. `toSignal` handles this neatly:
...
@Component({
  selector: 'app-legacy-amount',
  imports: [ReactiveFormsModule],
  template: `<input type="number" [formControl]="control" />`,
})
export class LegacyAmountComponent {
...
`control.valueChanges` is an Observable. `toSignal` turns it into a signal that participates in the reactive graph just like a `formField`. The reverse direction works too: a signal-based parent can patch a Reactive child's control imperatively with `control.setValue(...)` from an effect.

For new components, stick with Signal Forms. For legacy code, this interop pattern is usually enough to incrementally modernize the outer shell without rewriting every inner form.
