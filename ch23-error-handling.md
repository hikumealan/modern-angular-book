...
> **Forward reference:** The `ErrorBoundary` component and error display patterns built here are consumed by [Chapter 27](ch27-material-design-system.md)'s `FinFormField` wrapper, which uses them for inline validation error display.
...
On the server side, Angular's SSR runtime automatically installs `unhandledRejection` and `uncaughtException` listeners on the Node.js process. These forward server-side errors to the same `ErrorHandler`, giving you a unified error pipeline across both environments. See [Chapter 17](ch17-defer-ssr-hydration.md) for more on SSR architecture.
...
Register the interceptor alongside any existing ones in [Chapter 12](ch12-initialization-routes.md)'s configuration:
