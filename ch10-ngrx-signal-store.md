# Chapter 12: State Management with NgRx Signal Store
...
Hooks are particularly useful for triggering initial data loads. When the store is provided at the route level (as recommended in [Chapter 11](ch09-architecture.md)), `onInit` fires when the user navigates to that route, and `onDestroy` fires when they navigate away.
...
In [Chapter 13](ch10-http-resource.md), we will examine how Angular's `resource()` and `httpResource()` APIs integrate with signal stores to provide a fully declarative data-fetching layer.
