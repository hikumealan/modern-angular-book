...
Angular provides a layered set of tools for measuring and optimizing both loading performance (how quickly the application becomes interactive) and runtime performance (how smoothly it responds once running). Many of the building blocks -- lazy routes ([Chapter 4](ch04-router.md)), `@defer` blocks and SSR ([Chapter 17](ch17-defer-ssr-hydration.md)), signals ([Chapter 2](ch02-signal-components.md)) -- have appeared in earlier chapters. This chapter focuses on measurement, profiling, and the optimization techniques that tie those pieces together.
...
Route-level lazy loading ([Chapter 4](ch04-router.md)) uses `loadComponent` to split code at route boundaries. What is less obvious is that `loadComponent` runs in an injection context, which means you can inject services and make runtime decisions about which component to load.

FinancialApp offers a premium dashboard for paid users and a simpler version for free-tier users. Rather than shipping both bundles to everyone, the route conditionally loads the appropriate component:
...
export const DASHBOARD_ROUTES: Routes = [
  {
    ...
    loadComponent: () => {
      ...
      if (flags.isEnabled('premium-dashboard')) {
        return import('./premium-dashboard.component')
          .then(m => m.PremiumDashboardComponent);
      }

      return import('./basic-dashboard.component')
        .then(m => m.BasicDashboardComponent);
    },
  },
];
```

The `inject()` call works because `loadComponent` executes inside Angular's injection context during route resolution. The dynamic `import()` returns a promise, so Angular waits for the selected chunk to download before activating the route. Free-tier users never download the premium dashboard code, and premium users never download the basic version.
...
**Largest Contentful Paint (LCP)** measures how long it takes for the largest visible element to render. For FinancialApp, this is typically the account balance or the portfolio summary card. `NgOptimizedImage` with the `priority` attribute directly improves LCP by preloading the hero image. SSR ([Chapter 17](ch17-defer-ssr-hydration.md)) improves LCP by sending rendered HTML instead of an empty shell.
...
- **Bundle budgets** catch size regressions in CI. Use `--stats-json` with a bundle visualizer to diagnose what is consuming space.
- **Conditional lazy loading** with `loadComponent` and `inject()` lets routes load different components based on runtime state, keeping bundles focused on what each user actually needs.
