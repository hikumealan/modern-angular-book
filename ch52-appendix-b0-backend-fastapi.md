# Appendix B0: End-to-End Contract-Driven Angular Workflow

This appendix is the hands-on companion to [Chapter 24](ch27-advanced-typescript-openapi.md)'s "Closing the Loop with a FastAPI Backend" section. The chapter explains *why* a contract-driven workflow matters; this appendix shows *how* to build one, with working scripts, Nx library layout, and a CI pipeline you can copy.
...
- [Appendix B1: Backend Adapter -- Node + Express](ch53-ch53-ch53-appendix-b1-backend-express.md)
- [Appendix B2: Backend Adapter -- Spring Boot](ch54-ch54-ch54-appendix-b2-backend-spring.md)
- [Appendix B3: Backend Adapter -- Go](ch55-ch55-ch55-appendix-b3-backend-go.md)
...
// See ch52-appendix-b0-backend-fastapi.md -- "Strategy A"
...
// See ch52-appendix-b0-backend-fastapi.md -- "Strategy B"
...
// See ch52-appendix-b0-backend-fastapi.md -- "Strategy C"
import { execSync } from 'node:child_process';
...
// See ch52-appendix-b0-backend-fastapi.md -- "Wiring generated schemas into Signal Forms"
...
// See ch52-appendix-b0-backend-fastapi.md -- "Wiring generated schemas into DynamicFormComponent"
...
See [Chapter 7](ch07-testing-vitest.md) for Vitest setup and [Chapter 19](ch37-e2e-playwright.md) for the Playwright baseline.
...
**Accessibility + schema in one spec.** Combines the [Chapter 8](ch24-accessibility.md) axe-core integration with schema validation:
...
- **Node (Express) -> [Appendix B1](ch53-ch53-ch53-appendix-b1-backend-express.md)** -- TypeScript monorepo-friendly. Primary path uses `@asteasolutions/zod-to-openapi`, which unlocks the monorepo superpower of having the frontend and backend import the exact same Zod package. `tsoa` as a decorator-first alternative.
- **Java (Spring Boot) -> [Appendix B2](ch54-ch54-ch54-appendix-b2-backend-spring.md)** -- enterprise default. `springdoc-openapi` + Jakarta Bean Validation on Java records. Notes the Gradle/Maven integration path for Strategy D when publishing the Angular library as part of a JVM build.
- **Go (Gin / Echo / chi) -> [Appendix B3](ch55-ch55-ch55-appendix-b3-backend-go.md)** -- `oapi-codegen` for spec-first workflows, `swaggo/swag` for code-first. Covers `omitempty` semantics and integer/time format standardization.
...
- **Auth schemes.** The generated Angular services (Strategies A, C, D) all accept a `Configuration` object with `credentials` / `accessToken` hooks. A JWT bearer example using [Chapter 27](ch17-auth-patterns.md)'s patterns applies to every backend without changes.
...
- **Polyglot monorepos with Nx.** Nx custom executors (optionally augmented by community plugins like `@nxtension/python` where stable) let you drive all four strategies and the backend build from one `nx run-many -t build test lint` invocation. See [Chapter 37](ch15-advanced-nx.md).
...
- **Node (Express):** all four work. B is special: a shared Zod package makes the diff a no-op (see [Appendix B1](ch53-ch53-ch53-appendix-b1-backend-express.md)).
- **Java (Spring Boot):** all four work. D is particularly natural because OpenAPI Generator has deep Gradle/Maven integration (see [Appendix B2](ch54-ch54-ch54-appendix-b2-backend-spring.md)).
- **Go:** all four work. Spec-first tools like `oapi-codegen` pair especially well with Strategy B (see [Appendix B3](ch55-ch55-ch55-appendix-b3-backend-go.md)).
...
- [Chapter 8](ch24-accessibility.md) -- Combining axe-core with schema validation in E2E specs.
- [Chapter 40](ch19-error-handling.md) -- Retry interceptors; how they layer on Strategy D's generated services.
- [Chapter 19](ch37-e2e-playwright.md) -- Playwright setup, page objects, and CI integration.
- [Chapter 37](ch15-advanced-nx.md) -- Custom Nx executors for orchestrating the full regenerate-test pipeline.
