# Chapter 46: Compliance for Financial Apps
...
The `ConsentService` is the source of truth for what the user has agreed to. Every downstream consumer -- the telemetry service from [Chapter 42](ch42-observability.md), the marketing tag manager, the optional personalization features -- reads its state from here.
...
- **Consent is code.** The `ConsentService` is the single source of truth; the telemetry gate from [Chapter 42](ch42-observability.md) and every marketing script read its state before acting. The `ConsentBanner` is accessible by construction.
...
- **Jurisdictional disclosures ride on top of i18n ([Chapter 15](ch15-internationalization.md)).** Each locale carries the regulatory language required by its regime.
