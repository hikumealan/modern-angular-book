# Chapter 20: Command Palette & Keyboard Shortcuts
...
The palette is a dialog-style overlay rendered through `@angular/cdk/overlay`. An input at the top drives a result list grouped by category. Up/Down moves the highlight, Enter executes, Escape closes, Tab is trapped inside the dialog. The component uses the [Chapter 32](ch22-material-design-system.md) design system for visuals -- `FinIcon` for glyphs and token-backed SCSS for colors and spacing.
...
A command palette that only works with a mouse defeats its own purpose. [Chapter 8](ch24-accessibility.md) established the building blocks; the palette composes them into the most ARIA-dense widget in the application.
...
- **`role="listbox"` with `role="option"` children** maps the palette onto the WAI-ARIA combobox-with-listbox-popup pattern, the same contract `@angular/aria` Combobox from [Chapter 8](ch24-accessibility.md) implements for `FinAccountSelector`.
...
A complete walkthrough with VoiceOver -- opening, searching, executing, typing-and-hearing -- belongs in [Chapter 8](ch24-accessibility.md). Running that script against the palette uncovers any gaps before shipping.

---

## Testing

Following the pyramid from [Chapter 19](ch37-e2e-playwright.md), **unit tests** cover the registry and fuzzy search as plain TypeScript -- assertions on pure functions and signal state:
...
**Virtualize long result lists** when the registry exceeds a couple hundred items -- typically because it includes per-entity commands like "Open transaction T-12345." The CDK `ScrollingModule` from [Chapter 9](ch32-performance.md) is the drop-in answer: replace the inner `<ul>` with `<cdk-virtual-scroll-viewport itemSize="40">` plus `*cdkVirtualFor`. Keyboard navigation still works because the viewport scrolls the active row into view through `scrollToIndex`, called from the same `effect()` that resets the index.
...
- **`FinCommandPalette`** is a dialog-role overlay with a fuzzy-searching combobox input and grouped listbox, built on `@angular/cdk/overlay` and `@angular/cdk/a11y` and styled with the [Chapter 32](ch22-material-design-system.md) design system.
...
- **Accessibility** composes `role="dialog"`, `cdkTrapFocus`, `role="combobox"` with `aria-activedescendant`, and `LiveAnnouncer` announcements -- cross-referenced against [Chapter 8](ch24-accessibility.md) and exercised with screen readers per [Chapter 8](ch24-accessibility.md).
- **Testing** layers unit tests for the registry and fuzzy scoring plus E2E tests for the keyboard contract (Playwright, [Chapter 19](ch37-e2e-playwright.md)). Debounce the input and reach for CDK virtual scroll from [Chapter 9](ch32-performance.md) only when the registry exceeds a few hundred entries.
