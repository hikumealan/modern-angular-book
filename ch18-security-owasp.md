# Chapter 35: Security & OWASP Best Practices
...
FinancialApp uses the BFF pattern described in [Chapter 27](ch17-auth-patterns.md), which eliminates client-side token storage entirely. For applications that must store tokens in the browser, `sessionStorage` is marginally better (cleared when the tab closes) but still vulnerable to XSS. `HttpOnly` cookies are the only browser-native storage mechanism immune to JavaScript theft.
...
On the frontend, security-relevant events are captured and forwarded through the `GlobalErrorHandler` (detailed in [Chapter 40](ch19-error-handling.md)):
...
FinancialApp addresses this with two principles. First, server errors are never shown to the user verbatim. The `GlobalErrorHandler` (see [Chapter 40](ch19-error-handling.md)) maps HTTP error responses to safe, generic messages:
...
- **Token storage matters.** The BFF pattern from [Chapter 27](ch17-auth-patterns.md) eliminates client-side token storage entirely. If you must store tokens in the browser, `HttpOnly` cookies are the only option immune to XSS (A04).
