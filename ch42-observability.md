# Chapter 42: Observability & Production Monitoring
...
The interceptor pattern from [Chapter 12](ch12-initialization-routes.md) attaches an `X-Correlation-Id` header to every outbound request:
...
Register it near the start of the interceptor chain so retried or re-authenticated requests still carry the same header. The order places `correlationInterceptor` before the auth and retry interceptors from [Chapter 16](ch16-auth-patterns.md):
...
[Chapter 24](ch24-performance.md) introduced Core Web Vitals as design constraints measured in the lab. In production, you measure them on real user devices, which vary more than Lighthouse can simulate. The `web-vitals` library exposes each metric as an event callback that fires when the browser has enough data.
...
- **Sentry** is the default for Angular error monitoring. Compose `Sentry.createErrorHandler()` with the `GlobalErrorHandler` from [Chapter 23](ch23-error-handling.md) rather than replacing it, so structured logging and analytics keep working alongside Sentry's capture.
...
- **Releases and source maps** turn a stack trace into a fixable bug. Match the `release` identifier across `Sentry.init`, `sentry-cli releases new`, and `sentry-cli sourcemaps upload` in CI ([Chapter 41](ch41-ci-cd.md)).
- **Correlation IDs** generated once per session and attached via an HTTP interceptor ([Chapter 12](ch12-initialization-routes.md)) stitch browser events to backend logs. One `X-Correlation-Id` header collapses cross-service debugging from hours to minutes.
...
- **Privacy and residency** ([Chapter 46](ch46-gdpr-compliance.md)) require regional endpoint routing, consent gating for non-essential telemetry, and anonymization at source.
