...
Most teams face the other migration: a working Angular codebase stuck three or four major versions behind. The framework has shipped good migration tooling for years, and the Nx workflow from [Chapter 37](ch15-advanced-nx.md) makes even multi-major jumps tractable.
...
| Redux / Zustand | NgRx Signal Store, [Chapter 12](ch10-ngrx-signal-store.md) |
...
Feature flags ([Chapter 38](ch21-feature-flags.md)) wrap the new and old code paths:
...
**Automated upgrade PRs.** The `nx migrate` workflow from [Chapter 37](ch15-advanced-nx.md) runs weekly, creating a dependencies PR that is either green (merge) or red (investigate). Dependabot handles the non-Angular packages on the same cadence. Review one dependency PR per week and the framework stays current without a dedicated quarter of upgrade work.
...
- **Flag-gate behavior changes, not code changes.** Wrap user-visible differences behind feature flags from [Chapter 38](ch21-feature-flags.md). Keep code-shape migrations (decorator to signal API) flag-free -- the compiler catches issues that flags would hide.
...
- **Schematics do 90%.** The other 10% is where the interesting bugs live. Budget review time per migration commit equal to at least the generation time, and lean on the AI tooling from [Chapter 49](ch47-ai-in-angular.md) -- Cursor rules, the Angular CLI MCP, and agent skills know about `@angular/upgrade`, know which schematic replaces which decorator, and can read your `migrations.json` and explain what each entry does.
