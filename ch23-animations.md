...
> **Extends:** [Chapter 32](ch22-material-design-system.md) established the design-system foundation -- tokens, typography, Material setup, and wrapper components -- that this chapter's motion work layers onto. Start here for design-system primitives, then go deeper into route transitions, stagger patterns, callbacks, and reduced-motion accessibility.

---

## From the Design System

Angular's animation system provides a declarative API for transitions tied to component state. While CSS transitions handle simple hover effects, Angular animations integrate with the component lifecycle -- they trigger on route changes, respond to signal state, and coordinate staggered sequences across dynamic lists. The motion duration and easing tokens defined alongside the other design tokens in [Chapter 32](ch22-material-design-system.md) feed directly into these triggers.
...
**Toast slide-in.** The error toast container from [Chapter 40](ch19-error-handling.md) applies `@toastSlideIn` on each toast element. Because the `@for` block tracks by `toast.id`, Angular correctly identifies which toasts are entering and leaving.
...
These triggers cover the common cases; the rest of the chapter goes further -- composing state machines, choreographing route transitions with per-route metadata, respecting reduced-motion preferences, and testing everything without waiting real milliseconds.

---

## Composing Triggers Beyond a Single Transition

The `fadeIn` and `dialogEnterLeave` triggers from the previous section each handle one or two transitions. Real component states are messier. A transaction row can be `pending`, `cleared`, or `reverted`. A sync banner can be `idle`, `syncing`, `success`, or `error`. Each state combination may want its own animation. Angular's DSL composes cleanly up to that complexity, but only if you understand what the building blocks actually do.
...
The rule of thumb from [Chapter 8](ch24-accessibility.md)'s accessibility principles: animation that *conveys information* should stay (perhaps in a reduced form); animation that is *decorative* should disappear when reduced motion is requested.
...
This is a sledgehammer, and it is appropriate as a baseline. CSS transitions from [Chapter 32](ch22-material-design-system.md) go to effectively zero duration; nothing animates, nothing shifts. But it does not reach Angular's animation DSL, which compiles to `Element.animate()` calls that bypass CSS transition rules.
...
Platform-guarding via `isPlatformBrowser` keeps the service SSR-safe (see [Chapter 22](ch31-defer-ssr-hydration.md)). On the server, the signal defaults to `false`, and the real preference is picked up on hydration.
...
For end-to-end tests (see [Chapter 19](ch37-e2e-playwright.md)), Playwright can disable animations globally:
