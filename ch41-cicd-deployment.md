...
There is no universal "best" deployment target for an Angular app. The right answer depends on whether the app is a SPA or uses SSR ([Chapter 17](ch17-defer-ssr-hydration.md)), how much control you need, and what infrastructure your organization already runs.
...
A common hybrid pattern runs the static SPA on a CDN-backed host and the SSR rendering on containers. Angular's hybrid routing from [Chapter 17](ch17-defer-ssr-hydration.md) -- prerendered routes as static HTML, server-rendered routes as SSR -- fits this split naturally.
...
Chapter 42 covers the full observability story -- structured logging, distributed tracing, and error budgets. The hook from CI is narrow: every deploy announces itself to the tools that watch production, so those tools can connect deploys to the behavior that follows.
...
- **`nx affected` on every commit**, not the full workspace -- feedback stays close to the diff and the pipeline scales sub-linearly as the monorepo grows ([Chapter 30](ch30-advanced-nx.md)).
...
The pipeline here is the scaffolding on top of which every other quality investment in the book pays off. The Playwright suite from [Chapter 25](ch25-e2e-playwright.md) is only as valuable as how quickly its results reach the PR. The SSR architecture from [Chapter 17](ch17-defer-ssr-hydration.md) is only as useful as the container that serves it in production. The Nx monorepo from [Chapter 14](ch14-monorepos-libraries.md) only stays fast with the caching wiring in this chapter. Deployment is the seam where code meets users -- keep that seam tight, boring, and well-instrumented, and the team ships faster with less anxiety.
