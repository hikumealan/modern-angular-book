...
Selection must also be announced. Without it, a screen reader user activates the label, the native picker opens and closes, and they get no feedback on whether a file was chosen. Pipe selection and rejection through the `LiveAnnouncer` from [Chapter 8](ch24-accessibility.md):
...
- **Accessibility** uses a visually hidden input inside a styled `<label>`, paired with `LiveAnnouncer` announcements of selections and rejections, following [Chapter 8](ch24-accessibility.md).
...
- **CSV import** combines `papaparse` with Zod schemas from [Chapter 24](ch27-advanced-typescript-openapi.md) for row-level validation, surfacing errors per row so users fix individual problems. The flow composes with the transaction form from [Chapter 6](ch06-signal-forms.md).
