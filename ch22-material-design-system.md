# Chapter 32: Angular Material & Design System
...
This chapter builds the FinancialApp design system: tokens, typography, iconography, theming, Angular Material integration, domain-aware wrapper components, and animations. The headless Angular Aria components from [Chapter 8](ch24-accessibility.md) are wrapped with design tokens and domain logic here. The error display patterns from [Chapter 40](ch19-error-handling.md) are consumed by `FinFormField`.

> **Prerequisites:** [Chapter 13](ch11-signal-queries.md) (signal queries), [Chapter 23](ch25-internationalization.md) (locale-aware formatting), [Chapter 8](ch24-accessibility.md) (Angular Aria, a11y defaults), [Chapter 40](ch19-error-handling.md) (error display patterns).
...
When `ariaLabel` is undefined, `aria-hidden="true"` makes the icon decorative. When provided, `role="img"` and `aria-label` make it informative. This follows [Chapter 8](ch24-accessibility.md)'s principle that the callsite decides an icon's semantic role.
...
The empty-state slot uses `contentChild()` to detect a template reference named `emptyState`, following the signal query patterns from [Chapter 13](ch11-signal-queries.md). Consumers define columns using Material's `matColumnDef` directives inside the projected content.

### FinFormField

`FinFormField` wraps `MatFormField` and integrates the error display patterns from [Chapter 40](ch19-error-handling.md). The `contentChild(NgControl)` query finds the projected form control, reads its validation state reactively, and maps error keys to human-readable messages:
...
`FinAccountSelector` composes the Angular Aria Combobox from [Chapter 8](ch24-accessibility.md) with account search, type-ahead filtering, and account-type icons:
...
Currency inputs are deceptively complex. The display must use locale-appropriate formatting from [Chapter 23](ch25-internationalization.md), but the form control must hold the raw numeric value:
...
For the dedicated treatment of animation primitives, route transitions, and motion tokens, see [Chapter 39](ch23-animations.md).
...
- **Headless + styled composition.** The Angular Aria Combobox from [Chapter 8](ch24-accessibility.md) handles ARIA semantics. `FinAccountSelector` adds domain logic and token-based styling on top.
